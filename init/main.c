    ...
    do {
        ...
        if (parameqn(line, p->str, n)) {
            if (p->early) {
                ...
                if (line[n] == '\0' || line[n] == '=') had_early_param = true;
...
static void *__init get_boot_config_from_initrd(size_t *_size) {
     ...
     */
    for (i = 0; i < 4; i++) {
        if (!memcmp(data, BOOTCONFIG_MAGIC, BOOTCONFIG_MAGIC_LEN)) goto found;
        data--;
    }
    return NULL;
...
#else
static void *__init get_boot_config_from_initrd(size_t *_size) { return NULL; }
#endif
...
static int __init xbc_snprint_cmdline(char *buf, size_t size,
    ...
    xbc_node_for_each_key_value(root, knode, val) {
        ret = xbc_node_compose_key_after(root, knode, xbc_namebuf,
                                         XBC_KEYLEN_MAX);
        if (ret < 0) return ret;
        ...
        xbc_array_for_each_value(vnode, val) {
             ...
             */
            q = strpbrk(val, " \t\r\n") ? "\"" : "";
            ret = snprintf(buf, rest(buf, end), "%s=%s%s%s ", xbc_namebuf, q,
                           val, q);
            if (ret < 0) return ret;
            buf += ret;
        }
...
static int __init bootconfig_params(char *param, char *val, const char *unused,
                                    void *arg) {
...
static void __init setup_boot_config(void) {
    ...
    if (!data) data = xbc_get_embedded_bootconfig(&size);
    ...
    if (err) initargs_offs = err - tmp_cmdline;

    if (!data) {
        /* If user intended to use bootconfig, show an error level message */
        if (bootconfig_found)
            pr_err(
                "'bootconfig' found on command line, but no bootconfig "
                "found\n");
        else
...
static void __init exit_boot_config(void) { xbc_exit(); }
...
static int __init warn_bootconfig(char *str) {
    pr_warn(
        "WARNING: 'bootconfig' found on the kernel command line but "
...
static int __init unknown_bootoption(char *param, char *val, const char *unused,
    ...
    if (sysctl_is_alias(param)) return 0;
    ...
    if (val) {
        ...
        for (i = 0; envp_init[i]; i++) {
            ...
            if (!strncmp(param, envp_init[i], len + 1)) break;
        }
        envp_init[i] = param;
    } else {
        /* Command line option */
...
static int __init init_setup(char *str) {
     ...
     */
    for (i = 1; i < MAX_INIT_ARGS; i++) argv_init[i] = NULL;
    return 1;
}
...
static void __init print_unknown_bootoptions(void) {
    ...
    for (p = &argv_init[1]; *p; p++) end += sprintf(end, " %s", *p);
    for (p = &envp_init[2]; *p; p++) end += sprintf(end, " %s", *p);

    /* Start at unknown_options[1] to skip the initial space */
    pr_notice(
        "Unknown kernel command line parameters \"%s\", will be passed to user "
        "space.\n",
        &unknown_options[1]);
...
static void __init early_numa_node_init(void) {
    ...
    for_each_possible_cpu(cpu) set_cpu_numa_node(cpu, early_cpu_to_node(cpu));
#endif
#endif
}

asmlinkage __visible __init __no_sanitize_address __noreturn
    __no_stack_protector void
    start_kernel(void) {
    char *command_line;
    char *after_dashes;
    ...
    if (!IS_ERR_OR_NULL(after_dashes))
        parse_args("Setting init args", after_dashes, NULL, 0, -1, -1, NULL,
                   set_init_arg);
    if (extra_init_args)
        parse_args("Setting extra init args", extra_init_args, NULL, 0, -1, -1,
                   NULL, set_init_arg);
    ...
    anon_vma_init();
#ifdef CONFIG_X86
    if (efi_enabled(EFI_RUNTIME_SERVICES)) efi_enter_virtual_mode();
#endif
...
static int __init initcall_blacklist(char *str) {
    ...
    do {
        ...
        if (str_entry) {
            ...
            entry->buf =
                memblock_alloc_or_panic(strlen(str_entry) + 1, SMP_CACHE_BYTES);
...
static bool __init_or_module initcall_blacklisted(initcall_t fn) {
    ...
    if (list_empty(&blacklisted_initcalls)) return false;
...
static __init_or_module void trace_initcall_start_cb(void *data,
                                                     initcall_t fn) {
...
static __init_or_module void trace_initcall_level_cb(void *data,
                                                     const char *level) {
...
static int __init set_debug_rodata(char *str) {
    if (arch_parse_debug_rodata(str)) return 0;
